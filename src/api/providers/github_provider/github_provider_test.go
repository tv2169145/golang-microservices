...
func TestCreateRepoErrorRestClient(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Err: errors.New("invail restclient response"),
	})
	response, err := CreateRepo("", github.CreateRepoRequest{})
	assert.Nil(t, response)
	assert.NotNil(t, err)
	assert.EqualValues(t, "invail restclient response", err.Message)
}
...
func TestCreateRepoInvalidSuccessResponse(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.EqualValues(t, http.StatusInternalServerError, err.StatusCode)
	assert.EqualValues(t, "error unmarshal successful response", err.Message)
}
...
func TestCreateRepoSuccess(t *testing.T) {
	...
	assert.EqualValues(t, "golang-rest-api", response.Name)
}
